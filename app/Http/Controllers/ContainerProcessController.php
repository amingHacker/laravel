...
class ContainerProcessController extends Controller
{
    //This is the controller index
    public function index( Request $request )
    {      
        $todosContainer_Process = DB::connection('mysql')->table('container_process')->orderBy('id','desc')->first();
        
        return view('Container.ContainerProcess',[
            'todosContainer_Process' => $todosContainer_Process,
        ]);
    }
    ...
    {    
        ...
        {
            case "/ContainerProcess/show/Container_Process":
    ...
    {        
        ...
        {     
            ...
            //dd($Record);
            $DownLoadValue = DB::connection('mysql')->table($table)->limit(10000)->orderBy($sidx,'asc')->get();
    ...
    public function FileUpload( Request $request )
    {
        ...
        // //在鋼瓶秤重紀錄表裏操作
        // if ($uploadData["table"] == 'dgContainer_process')
        // {
        //     //從鋼瓶空重裡面撈出鋼瓶的空重
        //     if ($uploadData["UploadData"]["bottle_number"] !='' && $uploadData["UploadData"]["container_base_weight_ideal"] == '')
        //     {
        //         $tmp = DB::connection('mysql')->table("container_baseweight")->select('bottle_weight')
        //         ->where("bottle_number", $uploadData["UploadData"]["bottle_number"])->first(); 
        //         if ($tmp!=null)
        //         {
        //             $uploadData["UploadData"]["container_base_weight_ideal"] = $tmp->bottle_weight;
        //         }
        //         else
        //         {
        ...
        //         if ($tmp!=null)
        //         {
        //             $uploadData["UploadData"]["container_addpackaging_weight_ideal"] = $tmp->packaging_weight;
        //         }
        //         else
        //         {
        //             $uploadData["UploadData"]["container_addpackaging_weight_ideal"] = '';
        //         }   
        //     }
        // }
        ...
        $updateData = DB::connection('mysql')->table($table)->where("id", $uploadData["UploadData"]["id"]);
        $isExist = DB::connection('mysql')->table($table)->where("id", $uploadData["UploadData"]["id"])->first();

        if (array_key_exists("Return_Date", $uploadData["UploadData"])) {
            if ($uploadData["UploadData"]["Return_Date"] ==''){$uploadData["UploadData"]["Return_Date"] = NULL;}
        }
        if (array_key_exists("Working_Date", $uploadData["UploadData"])) {
            if ($uploadData["UploadData"]["Working_Date"] ==''){$uploadData["UploadData"]["Working_Date"] = NULL;}
        }
        if (array_key_exists("time1", $uploadData["UploadData"])) {
            if ($uploadData["UploadData"]["time1"] ==''){$uploadData["UploadData"]["time1"] = NULL;}
        }
        if (array_key_exists("time2", $uploadData["UploadData"])) {
            if ($uploadData["UploadData"]["time2"] ==''){$uploadData["UploadData"]["time2"] = NULL;}
        }
        if (array_key_exists("time3", $uploadData["UploadData"])) {
            if ($uploadData["UploadData"]["time3"] ==''){$uploadData["UploadData"]["time3"] = NULL;}
        }
        if (array_key_exists("time4", $uploadData["UploadData"])) {
            if ($uploadData["UploadData"]["time4"] ==''){$uploadData["UploadData"]["time4"] = NULL;}
        }
        if (array_key_exists("time5", $uploadData["UploadData"])) {
            if ($uploadData["UploadData"]["time5"] ==''){$uploadData["UploadData"]["time5"] = NULL;}
        }
        if (array_key_exists("time6", $uploadData["UploadData"])) {
            if ($uploadData["UploadData"]["time6"] ==''){$uploadData["UploadData"]["time6"] = NULL;}
        }
        if (array_key_exists("time7", $uploadData["UploadData"])) {
            if ($uploadData["UploadData"]["time7"] ==''){$uploadData["UploadData"]["time7"] = NULL;}
        }
        if (array_key_exists("time8", $uploadData["UploadData"])) {
            if ($uploadData["UploadData"]["time8"] ==''){$uploadData["UploadData"]["time8"] = NULL;}
        }
        if (array_key_exists("time9", $uploadData["UploadData"])) {
            if ($uploadData["UploadData"]["time9"] ==''){$uploadData["UploadData"]["time9"] = NULL;}
        }
        if (array_key_exists("time10", $uploadData["UploadData"])) {
            if ($uploadData["UploadData"]["time10"] ==''){$uploadData["UploadData"]["time10"] = NULL;}
        }
        if (array_key_exists("time11", $uploadData["UploadData"])) {
            if ($uploadData["UploadData"]["time11"] ==''){$uploadData["UploadData"]["time11"] = NULL;}
        }
        if (array_key_exists("time12", $uploadData["UploadData"])) {
            if ($uploadData["UploadData"]["time12"] ==''){$uploadData["UploadData"]["time12"] = NULL;}
        }
        if (array_key_exists("time13", $uploadData["UploadData"])) {
            if ($uploadData["UploadData"]["time13"] ==''){$uploadData["UploadData"]["time13"] = NULL;}
        }
        if (array_key_exists("time14", $uploadData["UploadData"])) {
            if ($uploadData["UploadData"]["time14"] ==''){$uploadData["UploadData"]["time14"] = NULL;}
        }
    ...
    {   
        ...
        $deleteData = DB::connection('mysql')->table($table)->delete($Parameter["id"]);
        $UpdateValue = DB::connection('mysql')->table($table)->orderBy('id','asc')->get(); //回傳原本的資料
        return response()->json([
            //'success' =>  $UpdateValue
            'success' => 'OK'
    ...
    {      
        ...
        // //在鋼瓶秤重紀錄表裏操作
        // if ($AddParameter["table"] == 'dgContainer_Process')
        // {
        //     //從鋼瓶空重裡面撈出鋼瓶的空重
        ...
        //         if ($tmp!=null)
        //         {
        //             $AddParameter["container_addpackaging_weight_ideal"] = $tmp->packaging_weight;
        //         }
        //         else
        //         {
        //             $AddParameter["container_addpackaging_weight_ideal"] = '';
        //         }   
        //     }
        // }
        ...
        if ($AddParameter["Return_Date"] ==''){$AddParameter["Return_Date"] = NULL;}
        if ($AddParameter["Working_Date"] ==''){$AddParameter["Working_Date"] = NULL;}
        if ($AddParameter["time1"] ==''){$AddParameter["time1"] = NULL;}
        if ($AddParameter["time2"] ==''){$AddParameter["time2"] = NULL;}
        if ($AddParameter["time3"] ==''){$AddParameter["time3"] = NULL;}
        if ($AddParameter["time4"] ==''){$AddParameter["time4"] = NULL;}
        if ($AddParameter["time5"] ==''){$AddParameter["time5"] = NULL;}
        if ($AddParameter["time6"] ==''){$AddParameter["time6"] = NULL;}
        if ($AddParameter["time7"] ==''){$AddParameter["time7"] = NULL;}
        if ($AddParameter["time8"] ==''){$AddParameter["time8"] = NULL;}
        if ($AddParameter["time9"] ==''){$AddParameter["time9"] = NULL;}
        if ($AddParameter["time10"] ==''){$AddParameter["time10"] = NULL;}
        if ($AddParameter["time11"] ==''){$AddParameter["time11"] = NULL;}
        if ($AddParameter["time12"] ==''){$AddParameter["time12"] = NULL;}
        if ($AddParameter["time13"] ==''){$AddParameter["time13"] = NULL;}
        if ($AddParameter["time14"] ==''){$AddParameter["time14"] = NULL;}
        ...
        else
        {
            ...
            unset($AddParameter["caption"]);
            $updateData = DB::connection('mysql')->table($table)->where('id', $request->id);
    ...
    {    
        ...
        switch ($_SERVER["REDIRECT_URL"])
        {
            case "/ContainerProcess/GetTable/Container_Process":
                $table = 'container_process';
                break;
        }
        
        $product_SPEC= DB::connection('mysql')->table($table)->get();
    ...
    {
        $todo = DB::connection('mysql')->table($table)->insert(
            $RowData 
        );
    }
    ...
    {    
        //dd($request);    
        $Company = DB::connection('mysql')->table('container_process')->select('Company')->distinct()->get();
    ...
    {  
        ...
        switch ($_SERVER["REDIRECT_URL"])
        {
            case "/ContainerProcess/GetDataFromID/Container_Process":
                $table = 'container_process';
                break;
        }
