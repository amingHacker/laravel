		...
		var ts= this, grid={
			...
			dragEnd: function() {
				...
				if(this.resizing) {
					...
					if($.isFunction(p.resizeStop)) { p.resizeStop.call(ts,nw,idx); }  //Aming 0526
				}
				this.curGbox = null;
		...
		*/
		...
		this.createTableRowForRule = function(rule, group ) {
			...
			ruleFieldSelect.bind('change',function() {
				...
				cm.searchoptions.searchColName = cm.name; //Aming
				...
				$(".input-elm",trpar).bind('change',function( e ) {
					...
					if(rule.data == undefined){rule.data = e.owner._oldvalue;}  //Aming
					that.onchange(); // signals that the filter has changed
				});
